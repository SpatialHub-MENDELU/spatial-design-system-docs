...
export class LessonDetailService {
  ...
  private _loadSessionData(lessonId) {
    ...
    this._state.completedIn =
      lessonByCourseVariant(this._state.activeCourse?.slug ?? '', currentLesson)
        ?.completed ?? null;
  }
  ...
  private _setLessonNavigation(lessonId) {
    ...
    if (currentLessonId < lessonsData.length) {
      ...
      this._state.successDialogText =
        'You have successfully completed the course. Great job!';
    ...
    const previousLesson = this._state.lessonsFromSession.find(
    ...
    this._state.canBeDisplayed = Boolean(
      currentLessonId === 1 ||
        (completedLessonsFromSession &&
          (lessonByCourseVariant(
            String(this._state.activeCourse?.slug),
            currentLesson
          )?.completed ||
            lessonByCourseVariant(
              String(this._state.activeCourse?.slug),
  ...
  addLessonToSession() {
    if (this._sessionService) {
      addCurrentLessonToSession(
        ...
        this._state.lessonById!,
        this._state.activeCourse!
      );
    }
  }
  ...
  async submitTask() {
    ...
    try {
      const data = await this._webContainersService?.checkTask(
        this._state.activeCourse?.type ?? ProjectType.VANILLA,
        test
      );
      if (data?.errors) {
