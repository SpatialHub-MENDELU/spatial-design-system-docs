...
export class FolderService {
  ...
  public static getInstance(webContainersService: WebContainerService): FolderService {
    if (!this.instance) {
  ...
  public async createNewItem(name: string, type: FileType, parentFolder?: FolderItem) {
    ...
    const isDuplicate = parentFolder 
      ? parentFolder.children?.some(child => child.name === newItem.name) 
      : this.folders.value.some(folder => folder.name === newItem.name);

    if (isDuplicate) {
      return { error: `An item with the name "${newItem.name}" already exists.` };
    }
    ...
    try {
      if (type === FileType.FOLDER) {
        ...
        await this.webContainersService?.writeFile(`/${parentFolder.name}/${newItem.name}`, '');
      } else {
        this.folders.value.push(newItem);
        await this.webContainersService?.writeFile(`/${newItem.name}`, ''); // Create file at root
      }
