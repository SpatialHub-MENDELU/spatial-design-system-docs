...
const onFolderClick = (node: TreeNode) => {
  if (
    ...
    `${node.key !== '/' ? node.key : ''}/${props.selectedItem?.label}`
  )
    return;
...
const moveFile = async () => {
  ...
  try {
    await nextTick();
    const oldFilePath = props.selectedItem?.key;
    ...
    playgroundStore.dispatch('moveFile', {
      oldFile: {
        name: props.selectedItem?.label,
        path: oldFilePath,
        type: props.selectedItem?.type
      },
      newFileName: props.selectedItem?.label,
      path: `${selectedFolder.value?.key}/${props.selectedItem?.label}`,
    });
  } catch (error) {
    toast.add({
      severity: 'error',
    ...
    >
      <template #default="{ node }">
        <span
          ...
          v-if="
            node.data.type === FileType.FOLDER &&
    ...
    <template #footer>
      ...
      <div class="flex justify-end mt-4 gap-3">
        ...
        <button
          ...
          :disabled="
            selectedFolder === null ||
            props.selectedItem?.key === selectedFolder.key
          "
          :class="{
            'opacity-50':
              selectedFolder === null ||
